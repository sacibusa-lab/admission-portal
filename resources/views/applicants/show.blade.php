                                        ...
                                        <div class="border rounded p-3 bg-light text-center">
                                            ...
                                            @elseif($average !== null)
                                                ...
                                                @if(auth()->user()->hasRole(['Super Admin', 'Admission Officer']) && !str_contains($applicant->exam_batch ?? '', 'Resit'))
                                                    <form action="{{ route('applicants.resit', $applicant->id) }}" method="POST" class="mt-2" onsubmit="return confirm('Are you sure you want to register this applicant for a resit exam? Existing scores will be preserved under the current batch.')">
                                                        @csrf
